...
def resolve_auth_headers(config):
    ...
    # Prefer slack-auth.json (client_id + access_token).
    auth_path = skill_dir / "slack-auth.json"
    if auth_path.exists():
    ...
    # Fallback to slack-token.json produced by slack_auth.py.
    token_path = skill_dir / "slack-token.json"
    ...
    print(
        ...
        "Create slack-auth.json from slack-auth.sample.json, set SLACK_MCP_TOKEN,\n"
        "or run ./slack_auth.py to authenticate.",
        file=sys.stderr,
    )
    sys.exit(1)
