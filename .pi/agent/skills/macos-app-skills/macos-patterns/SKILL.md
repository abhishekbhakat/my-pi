...
Sleep so the close animation finishes before the Dock icon vanishes. Reset to accessory at launch.
...
Carbon cannot see key-up for hold-to-talk. Use Tier 4.

### Tier 4: CGEvent tap (hold-to-talk, consume events)

Needs Accessibility. Re-enable on `tapDisabledByTimeout`. Store `CFMachPort` as `nonisolated(unsafe)` under Swift 6.
...
let tap = CGEvent.tapCreate(
    ...
    callback: { _, type, event, refcon -> Unmanaged<CGEvent>? in
        if type == .tapDisabledByTimeout || type == .tapDisabledByUserInput {
            if let tap = refcon.flatMap({ Unmanaged<HotkeyManager>.fromOpaque($0).takeUnretainedValue().eventTap }) {
                CGEvent.tapEnable(tap: tap, enable: true)
            }
            return Unmanaged.passUnretained(event)
        }
        // Match key + modifiers; return nil to consume, else passUnretained(event)
        return Unmanaged.passUnretained(event)
    },
...
`NSEvent.addGlobalMonitorForEvents` cannot consume. Do not use it for push-to-talk.

## Permissions (TCC)

Poll status only with APIs that never show a system sheet.

| Need | Status (no prompt) | Request |
| --- | --- | --- |
| Microphone | `AVAudioApplication.shared.recordPermission` | `requestRecordPermission` once from a button |
| Accessibility | `AXIsProcessTrusted()` | `AXIsProcessTrustedWithOptions` with prompt, once |
| Screen / system audio | `CGPreflightScreenCaptureAccess()` | `CGRequestScreenCaptureAccess()` or open Privacy pane |
| Login item | `SMAppService.mainApp.status` | `register()` / `unregister()` |

Never call `SCShareableContent` on a timer. That prompts. Never pass `prompt:true` while polling Accessibility.

`.requiresApproval` on login items: keep the toggle off until the user approves in System Settings. Skip `register()` in debug builds.

Usage strings live in `Info.plist` (`NSMicrophoneUsageDescription`, `NSScreenCaptureUsageDescription`). Entitlements: `com.apple.security.device.audio-input`, `com.apple.security.automation.apple-events`. Sparkle often needs `com.apple.security.cs.disable-library-validation` plus a nested-framework rpath.

## Keychain for secrets

API keys do not belong in `@AppStorage`. Generic password, delete-then-add:

```swift
let query: [String: Any] = [
    kSecClass as String: kSecClassGenericPassword,
    kSecAttrService as String: "com.example.api-keys",
...
## Type into another app (Accessibility)

Capture the focused PID **before** your overlay or policy change steals focus. Then activate that PID and post keystrokes.
...
var pid: pid_t = 0
AXUIElementGetPid(unsafeBitCast(ref!, to: AXUIElement.self), &pid)
...
Screen Recording TCC is still required. Do not capture video frames if you only need audio.

```swift
let config = SCStreamConfiguration()
config.capturesAudio = true
config.excludesCurrentProcessAudio = true
config.minimumFrameInterval = CMTime(value: 1, timescale: 1) // cheap video if the API still wants a stream
```
...
| `addEventListener('keydown')` for global shortcuts | Only works when app is focused | Carbon for press; CGEvent tap for hold-to-talk |
| Poll `SCShareableContent` for permission | Prompts TCC on a timer | `CGPreflightScreenCaptureAccess()` |
| Store API keys in `@AppStorage` | Plist on disk | Keychain generic password |
