...
1. Environment: `TYPESAFE_API_KEY`
2. `typesafe-auth.json` in this skill directory (gitignored)

```bash
cp typesafe-auth.sample.json typesafe-auth.json
# put key in api_key
```
...
HTTP: `Authorization: Bearer <API_KEY>` on `POST https://api.typesafe.ai/v1/systemone`.
Python SDK (`typesafe-sdk`) reads `TYPESAFE_API_KEY` from the environment.
If the key is missing or a call returns 401, stop: tell the user to add the key,
help write `typesafe-auth.json`, then continue. Do not retry blind.
