...
const TMP_DIR = IS_WINDOWS ? join(homedir(), '.cdp-tmp') : tmpdir();
const SOCK_PREFIX = IS_WINDOWS ? '\\\\.\\pipe\\cdp-' : join(TMP_DIR, 'cdp-');
const PAGES_CACHE = join(TMP_DIR, 'cdp-pages.json');

// Ensure TMP_DIR exists (needed on Windows where we use ~/.cdp-tmp)
try { mkdirSync(TMP_DIR, { recursive: true }); } catch {}

function sockPath(targetId) {
  if (IS_WINDOWS) return `${SOCK_PREFIX}${targetId}`;
...
function getWsUrl() {
  const candidates = [
    ...
    // Windows
    resolve(process.env.LOCALAPPDATA || '', 'Chromium/User Data/DevToolsActivePort'),
  ...
  for (const c of candidates) {
    if (existsSync(c)) { portFile = c; break; }
  }
  if (!portFile) throw new Error('No DevToolsActivePort found. Enable remote debugging in your browser.');
...
function listDaemonSockets() {
  if (IS_WINDOWS) {
    // On Windows we use named pipes; no directory listing available.
    // Return empty -- Windows always uses pages cache for target resolution.
    return [];
  }
  try {
...
async function shotStr(cdp, sid, filePath) {
  ...
  const out = filePath || join(TMP_DIR, 'screenshot.png');
