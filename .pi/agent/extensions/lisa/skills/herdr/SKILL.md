---
name: herdr
description: "Control Herdr, a terminal multiplexer for coding agents. Lisa pilot tool, always available in Lisa mode. Organize terminals into workspaces, tabs, and panes, inspect agents in panes, type into panes, and read pane output. Requires HERDR_ENV=1."
---
...
Give routine work through direct pane input. Session-to-session messages go through the pi-mail tool.

Before typing, the target must be settled. If it is not, wait with no `--until` so `idle`, `done`, and `blocked` all count:

```bash
herdr agent wait reviewer --timeout 120000
```

If the wait returns `blocked`, inspect `agent get` and `agent read`. Ask the user before answering the dialog. Do not type into a blocked agent.

Type plain instructions with no headers:

```bash
herdr pane run <pane-id> "Review the current diff. Report only actionable findings."
```

Then tell the user who has the work and end your turn. Do not poll. Read the result back with `pane read` when the other side finishes.

Sessions talk to each other with the pi-mail tool: `mail send` queues text, `mail inbox` peeks, `mail read` injects. Mail is passive and no turn triggers. Address a mailbox by full session id or unique prefix.

You hold no mail tool. Tell the sender pane exactly what to queue and where. Then poke the recipient pane with direct input so it runs `mail read`:

```bash
herdr pane run <pane-id> "Check your pi-mail inbox with mail read and act on new messages."
```
...
Without `--until`, standalone `agent wait` waits for the first settled `idle`, `done`, or `blocked` state.
